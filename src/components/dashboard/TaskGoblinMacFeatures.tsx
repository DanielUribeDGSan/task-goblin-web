...
export const TaskGoblinMacFeatures = ({
  appType = 'task-goblin',
  ...
  const heroRoom: RoomConfig = {
    id: 'general-intro',
    title: t.videoCardTitle || 'Task Goblin',
    distance: '',
    icon: config.heroIcon,
    aspectRatio: 'video',
    mediaItems: [
      { type: 'video', src: '/task-goblin/video/task-goblin.mp4', poster: '/task-goblin/images/task-goblin-general.png' },
      { type: 'image', src: '/task-goblin/images/task-goblin-general.png' }
    ]
  };
  ...
  const activeMedia = activeFeature ? activeFeature.mediaItems[0] : { type: 'image' as const, src: '/task-goblin/images/task-goblin-general.png', poster: '/task-goblin/images/task-goblin-general.png' };
  ...
  useEffect(() => {
    ...
    // The video element now handles its own play/pause state via onPlay/onPause events
    ...
    // Auto-scroll the newly opened accordion item into view
    if (activeId) {
      setTimeout(() => {
  ...
  }, [activeId, activeMedia.type]); // Fix blinking by only triggering on activeId change
  ...
  useEffect(() => {
    const updateHeight = () => {
      // Delay height calculation slightly to ensure CSS and images have loaded their dimensions
      setTimeout(() => {
  ...
  return (
    <div className="w-full h-full max-w-[1400px] mx-auto py-2 lg:py-6 px-4 sm:px-8 flex flex-col justify-center">
      ...
      {/* Container for Mac-style layout */}
      <div className="flex flex-col lg:flex-row-reverse gap-8 lg:gap-16 items-start">
        ...
        <div ref={laptopRef} className="w-full lg:w-[60%] shrink-0 relative">
          <div className="relative mx-auto max-w-[800px]">
                  ...
                  >
                    {activeMedia.type === 'video' ? (
                      ...
                      <img
                        src={activeMedia.src}
                        className="w-full h-full object-cover"
                        alt={activeFeature?.title || "Task Goblin"}
                      />
                    )}
                  </motion.div>
                </AnimatePresence>
            ...
            {/* Mobile/Tablet Floating Info Card (Hidden on LG) */}
            {/* On Mobile (< sm): Static below the laptop. On Tablet (sm to lg): Floating over laptop */}
                ...
                <div className="flex items-center gap-3">
                  {activeId && (
                    <div className="w-9 h-9 shrink-0 rounded-full bg-white/5 flex items-center justify-center p-2">
                      <img src={activeFeature?.icon || config.heroIcon} alt="" className="w-full h-full object-contain opacity-80" />
                    </div>
                  )}
                  <span className="text-white font-bold text-base pr-2">
                    {activeId === 'general-intro' 
                      ? heroRoom.title 
                      : (activeId 
                          ? (t.cardTitles[activeId as keyof typeof t.cardTitles] || activeFeature?.title) 
                          : (t.bottomBar.price === 'Precio' ? 'Selecciona una función' : 'Select a feature'))}
                  </span>
                </div>
                <button 
                ...
                <p className="text-sm text-[#A1A1A6] leading-relaxed">
                  {activeId === 'general-intro' 
                    ...
                    : (activeId 
                        ...
                        : (t.bottomBar.price === 'Precio' ? 'Toca el botón de menú a la derecha para ver y seleccionar las funciones que deseas explorar.' : 'Tap the menu button on the right to view and select the features you want to explore.'))}
                </p>
                
                {/* General Intro Buttons for Mobile */}
                {activeId === 'general-intro' && (
                  <div className="flex flex-col gap-3 mt-4">
                    ...
                    <div className="flex flex-col gap-2">
                       <button
                        type="button"
                        className="w-full flex items-center justify-center gap-2 rounded-xl bg-white/5 border border-white/10 px-3 py-2 hover:bg-white/10 transition-colors cursor-pointer"
                      >
                        <Sparkles size={16} className="shrink-0 text-[#2BE46A]" />
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-1 rounded-xl bg-[#007AFF] hover:bg-[#007AFF]/90 px-3 py-2 text-sm font-semibold text-white transition-colors cursor-pointer"
                      >
        ...
        >
          {features.map((feature) => {
            ...
            const descStr = feature.id === 'general-intro'
              ? t.permissionsCard.featuresTitle // Using this as a placeholder for intro text
            ...
            return (
              <div 
                ...
                key={feature.id} 
                className={`shrink-0 transition-all duration-300 rounded-3xl overflow-hidden ${
                  isActive ? 'bg-[#1C1C1E]' : 'bg-transparent hover:bg-[#1C1C1E]/50'
                }`}
              >
                <button
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 shrink-0 rounded-full bg-white/5 flex items-center justify-center p-2">
                      <img src={feature.icon} alt="" className="w-full h-full object-contain filter group-hover:brightness-110 transition-all opacity-80" />
                    </div>
                  ...
                  {isActive && (
                    ...
                    >
                      <div className="px-5 pb-6 pt-0 ml-14">
                        <p className="text-[15px] leading-relaxed text-[#A1A1A6] mb-4">
                          {descStr}
                        </p>
                        
                        {/* Only for General Intro show the permission buttons to replace legacy Hero */}
                        {feature.id === 'general-intro' && (
                          <div className="flex flex-col gap-3 mt-4">
                             <ul className="list-disc list-inside text-sm text-[#A1A1A6] space-y-1.5 mb-2">
                                {t.featuresList.slice(0, 4).map((item) => (
                            ...
                            <div className="flex flex-col xl:flex-row gap-2">
                               <button
                                type="button"
                                className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-white/5 border border-white/10 px-3 py-2 hover:bg-white/[0.08] transition-colors cursor-pointer"
                              >
                                <Sparkles size={16} className="shrink-0 text-[#2BE46A]" />
                                <span className="text-sm font-semibold text-white">
                                  {t.permissionsCard.getTaskGoblinPro}
                              ...
                              <button
                                type="button"
                                onClick={(e) => { e.stopPropagation(); setInfoModalOpen(true); }}
                                className="flex-1 flex items-center justify-center gap-1 rounded-xl bg-[#007AFF] hover:bg-[#007AFF]/90 px-3 py-2 text-sm font-semibold text-white transition-colors cursor-pointer"
                              >
                                {t.permissionsCard.getMoreInfo}
                              </button>
                            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-colors text-left ${
                      isActive ? 'bg-[#1C1C1E]' : 'bg-transparent hover:bg-white/5'
                    }`}
                  >
                    <div className="w-10 h-10 shrink-0 rounded-full bg-white/5 flex items-center justify-center p-2">
                      <img src={feature.icon} alt="" className="w-full h-full object-contain filter opacity-80" />
                    </div>
