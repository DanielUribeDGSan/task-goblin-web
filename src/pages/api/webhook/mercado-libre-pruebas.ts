...
export const POST: APIRoute = async ({ request }) => {
    try {
        ...
        if (type === 'payment' && id) {
            const accessToken = import.meta.env.ACCESS_TOKEN_PRUEBAS;
            ...
            try {
                ...
                if (paymentDetails && paymentDetails.status === 'approved') {
                    ...
                    console.log(`Processing approved PRUEBAS payment for ${userEmail} - App: ${appName} - Payment: ${id}`);

                    const autoLicenseKey = "MP-TEST-" + crypto.randomUUID().toUpperCase().split("-")[0] + "-" + Math.random().toString(36).substring(2, 11).toUpperCase();
